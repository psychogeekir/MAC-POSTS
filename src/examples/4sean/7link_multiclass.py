...
for i in range(config['num_data']):
    ...
    #                          np.arange(0, dode.num_loading_interval, dode.ass_freq) + dode.ass_freq, False).flatten(order = 'F')
...
(f_car, f_truck, x_e_car, x_e_truck, tt_e_car, tt_e_truck, O_demand, loss_list) = \
    dode.estimate_path_flow_pytorch(max_epoch = 50, algo='NAdam', normalized_by_scale = True,
                                    ...
                                    starting_epoch=0, store_folder=os.path.join(data_folder, 'record'))
...
nb.load_from_folder(os.path.join(data_folder))
