...
MNM_Dlink_Ctm::MNM_Dlink_Ctm(TInt ID,
    ...
    TFlt _lane_hold_cap_last_cell = MNM_Ults::max(
            ((m_length - TFlt(m_num_cells - 1) * _std_cell_length) / _std_cell_length) * m_lane_hold_cap,
            m_lane_hold_cap);
    TFlt _wave_speed =
            m_lane_flow_cap / (m_lane_hold_cap - m_lane_flow_cap / m_ffs); //laneFlwCap/ffs is the critical density.
    m_wave_ratio = _wave_speed /
                   m_ffs; // note that h >= 2c/v, where h is holding capacity, c is capcity, v is free flow speed. i.e., wvRatio should < 1.
    if (m_wave_ratio < 0) {
...
int MNM_Dlink_Ctm::clear_incoming_array(TInt timestamp) {
...
TFlt MNM_Dlink_Ctm::Ctm_Cell::get_supply() {
    ...
    if (m_wave_ratio <= 1.0) //this one is quite tricky, why not just _min(flwCap, hldCap - curDensity)*wvRatio?
        return m_flow_cap > _real_volume ? m_flow_cap : TFlt(
                (m_hold_cap - _real_volume) * m_wave_ratio);  //flowCap equals to critical density
    else
...
TFlt MNM_Dlink_Ctm::get_link_tt() {  // in seconds, not in unit time interval, can only be called in simulation, not after simulation
    TFlt _cost, _spd;
...
int MNM_Dlink_Pq::clear_incoming_array(TInt timestamp) {
...
int MNM_Dlink_Lq::clear_incoming_array(TInt timestamp) {
...
int MNM_Dlink_Ltm::clear_incoming_array(TInt timestamp) {
    // printf("MNM_Dlink_Ltm::clear_incoming_array\n");
    if (TInt(m_incoming_array.size()) > TInt(get_link_supply() * m_flow_scalar)) {
        printf("Error in MNM_Dlink_Ltm::clear_incoming_array()\n");
        exit(-1);
    }
    m_N_in2.add_increment(std::pair<TFlt, TFlt>(TFlt(m_current_timestamp * m_unit_time),
