...
public:
    ...
    virtual int update_path_table_fixed_departure_time_choice(MNM_Dta *dta, int iter) { return 0; };
...
public:
    ...
    virtual int update_path_table_fixed_departure_time_choice(MNM_Dta *dta, int iter) override;

    virtual int update_path_table_gp_fixed_departure_time_choice(MNM_Dta *dta, int iter) override;

    std::pair<MNM_Path *, TInt> get_best_route(TInt o_node_ID,
                                               MNM_TDSP_Tree *tdsp_tree);

    std::pair<MNM_Path *, TInt> get_best_route_for_single_interval(TInt interval,
                                                                   TInt o_node_ID,
                                                                   MNM_TDSP_Tree *tdsp_tree);
