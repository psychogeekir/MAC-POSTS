...
MNM_Dnode *MNM_Node_Factory::get_node(TInt ID)
{
  ...
  if (_node_it == m_node_map.end()) {
    ...
    throw std::runtime_error("Error, MNM_Node_Factory::get_node, node does not exist");
  }
...
MNM_Dlink *MNM_Link_Factory::get_link(TInt ID)
{
  ...
  if (_link_it == m_link_map.end()){
    throw std::runtime_error("Error, MNM_Link_Factory::get_link, link does not exist");
  }
...
MNM_Destination *MNM_OD_Factory::get_destination(TInt ID)
{
  ...
  if (_d_it == m_destination_map.end()){
    throw std::runtime_error("Error, MNM_OD_Factory::get_destination, destination does not exist");
  }
  return _d_it -> second;
}
...
MNM_Origin *MNM_OD_Factory::get_origin(TInt ID)
{
  ...
  if (_o_it == m_origin_map.end()){
    throw std::runtime_error("Error, MNM_OD_Factory::get_origin, origin does not exist");
  }
  return _o_it -> second;
}