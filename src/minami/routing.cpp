...
/**************************************************************************
                          Adaptive routing
**************************************************************************/
MNM_Routing_Adaptive::MNM_Routing_Adaptive(const std::string& file_folder, PNEGraph &graph, MNM_Statistics* statistics,
                  MNM_OD_Factory *od_factory, MNM_Node_Factory *node_factory, MNM_Link_Factory *link_factory)
  : MNM_Routing::MNM_Routing(graph, od_factory, node_factory, link_factory)
{
...
int MNM_Routing_Fixed::init_routing(Path_Table *path_table)
{
  if (path_table == nullptr && m_path_table == nullptr){
    printf("Path table not set, probably it needs to be set in Fixed routing.\n");
    // exit(-1);
  }
  ...
  for (auto _link_it = m_link_factory -> m_link_map.begin(); _link_it != m_link_factory -> m_link_map.end(); _link_it ++){
    ...
    for (auto _veh_it = _link -> m_finished_array.begin(); _veh_it != _link -> m_finished_array.end(); _veh_it++){
      ...
      if (_veh -> m_type == MNM_TYPE_STATIC){
        _veh_dest = _veh -> get_destination();
        // printf("2.2\n");
        if (_veh_dest -> m_dest_node -> m_node_ID == _node_ID){  // vehicles reaching destination
          if (m_tracker.find(_veh) -> second -> size() != 0){  // check if any links left in the route
...
**************************************************************************/
MNM_Routing_Hybrid::MNM_Routing_Hybrid(const std::string& file_folder, PNEGraph &graph, MNM_Statistics* statistics,
  MNM_OD_Factory *od_factory, MNM_Node_Factory *node_factory, MNM_Link_Factory *link_factory, 
...
{
  if (m_routing_adaptive -> m_working) m_routing_adaptive -> init_routing();
...
{
  if (m_routing_adaptive -> m_working) m_routing_adaptive -> update_routing(timestamp);
...
**************************************************************************/
MNM_Routing_Biclass_Hybrid::MNM_Routing_Biclass_Hybrid(const std::string& file_folder, PNEGraph &graph, MNM_Statistics* statistics,
  ...
  : MNM_Routing::MNM_Routing(graph, od_factory, node_factory, link_factory)
{
  ...
  m_routing_adaptive = new MNM_Routing_Adaptive(file_folder, graph, statistics, od_factory, node_factory, link_factory);
  auto *_tmp_config = new MNM_ConfReader(file_folder + "/config.conf", "DTA");
  m_routing_adaptive -> m_working = _tmp_config -> get_float("adaptive_ratio_car") > 0 || _tmp_config -> get_float("adaptive_ratio_truck") > 0;
  delete _tmp_config;
}
...
int MNM_Routing_Biclass_Fixed::change_choice_portion(TInt routing_interval)
{
    ...
    MNM::copy_buffer_to_p(m_path_table, routing_interval + m_veh_class * TInt(m_buffer_length / 2));
    MNM::normalize_path_table_p(m_path_table);
    return 0;
}
...
int MNM_Routing_Biclass_Fixed::update_routing(TInt timestamp)
{
  ...
  if (m_buffer_as_p){
    // if (timestamp % m_routing_freq  == 0 || timestamp == 0) {
      // printf("11\n");
      _cur_ass_int = TInt(timestamp / m_routing_freq);
      if (_cur_ass_int < TInt(m_buffer_length / 2)){  // first half for car, last half for truck
...
//      TInt tmp2 = _veh -> get_class(); // virtual getter of derived veh_multiclass
