        ...
        for (int e = 0; e < _node_it.GetInDeg(); ++e) {
            ...
            if (_tmp_ID != dest_node_ID) {
                ...
                if (_out_link_ID != -1 &&
                    ...
                    node_cost_map.find(_in_link_ID)-> second.find(_out_link_ID) != node_cost_map.find(_in_link_ID)-> second.end()) {
                    _alt += node_cost_map.find(_in_link_ID) -> second.find(_out_link_ID) -> second;
                }
            }
            // printf("Current alternative distance is %.4f\n", _alt());
            if (_alt < _dist.find(_in_node_ID)->second) {
        ...
        for (int e = 0; e < _node_it.GetInDeg(); ++e) {
            ...
            if (_tmp_ID != dest_node_ID) {
                ...
                if (_out_link_ID != -1 &&
                    node_cost_map.find(_in_link_ID) != node_cost_map.end() && 
                    node_cost_map.find(_in_link_ID)-> second.find(_out_link_ID) != node_cost_map.find(_in_link_ID)-> second.end()) {
                    _alt += node_cost_map.find(_in_link_ID) -> second.find(_out_link_ID) -> second[cost_position];
                }
            }
            // printf("Current alternative distance is %.4f\n", _alt());
            if (_alt < dist_to_dest[_in_node_ID][dist_position]) {
...
bool MNM_Shortest_Path::is_FIFO(PNEGraph graph, const std::unordered_map<TInt, TFlt *> &cost_map,
...
int MNM_TDSP_Tree::update_tree(const std::unordered_map<TInt, TFlt*>& link_cost_map, 
    ...
    // printf("SP in last t\n");
    // run last time interval
    MNM_Shortest_Path::all_to_one_FIFO(m_dest_node_ID, m_graph, link_cost_map, node_cost_map, m_dist, m_tree,
                                       m_max_interval - 1, m_max_interval - 1, m_max_interval - 1);
    // printf("Process M-2 to 0\n");
    // main loop for t = M-2 down to 0
    // construct m_dist and m_tree in a reverse time order
