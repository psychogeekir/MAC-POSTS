...
MNM_Statistics::MNM_Statistics(const std::string& file_folder, MNM_ConfReader *conf_reader, MNM_ConfReader *record_config,
...
MNM_Statistics_Lrn::MNM_Statistics_Lrn(const std::string& file_folder, MNM_ConfReader *conf_reader, MNM_ConfReader *record_config,
                                       MNM_OD_Factory *od_factory, MNM_Node_Factory *node_factory, MNM_Link_Factory *link_factory)
...
int MNM_Statistics_Lrn::init_record()
{
  ...
  if (m_record_volume){
    for (auto _link_it : m_link_factory -> m_link_map){
      _link_ID = _link_it.first;
      m_to_be_volume.insert(std::pair<TInt, TFlt>(_link_ID, TFlt(0)));  // intermediate variable to compute average quantities
    }
  }
  if (m_record_tt){
    for (auto _link_it : m_link_factory -> m_link_map){
